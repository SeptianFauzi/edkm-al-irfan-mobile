...
const ConfirmBeriZakatFitrah = ({ route, navigation }) => {
    ...
    const onSubmit = async (data) => {
        dispatch(UpdateZakatFitrahSent({ id_peserta: data.id, data: data })).then(() => {
            setTimeout(() => {
                navigation.goBack()
            }, 2000);
        }).catch(() => {
    ...
    useEffect(() => {
        if (state.zakatFitrahSent) {
            reset({
                ...
                amount_sent: state.zakatFitrahSent ? state.zakatFitrahSent[0].amount_sent : 0,
                is_zakat_sent: state.zakatFitrahSent ? state.zakatFitrahSent[0].is_zakat_sent : false,
    ...
    const dispatchData = () => {
        dispatch(GetDetailZakatFitrahSent({ id_peserta: id_peserta, year_hijriah: year_hijriah }))
    }
            ...
            < View style={styles.container} >
                <View style={styles.sub_container}>
                    ...
                    <Card style={styles.container_menu}>
                        <Card.Content>
                            <View style={{ flexDirection: 'column', justifyContent: 'space-around', width: '100%' }}>
                                <Title style={{ fontSize: 20, textAlign: 'center', color: 'white' }}> Identitas Peserta Penerima Zakat Fitrah</Title>
                                ...
                                <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between' }}>
                                    <View style={{ flexDirection: 'column' }}>
                                        <View style={{ flexDirection: 'row' }}>
                                            ...
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.zakatFitrahSent[0].id_peserta}</Text>
                                        </View>
                                        <View style={{ flexDirection: 'row' }}>
                                            <Text style={{ fontSize: 16, color: 'white' }}> Nama  </Text>
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.zakatFitrahSent[0].id_peserta_peserta.name}</Text>
                                        </View>
                                    </View>
                                    <View style={{ flexDirection: 'column' }}>
                                        <View style={{ flexDirection: 'row' }}>
                                            <Text style={{ fontSize: 16, color: 'white' }}> Tahun  </Text>
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.zakatFitrahSent[0].year_hijriah} H</Text>
                                        </View>
                                        <View style={{ flexDirection: 'row' }}>
                                            <Text style={{ fontSize: 16, color: 'white' }}> Jatah  </Text>
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.zakatFitrahSent[0].amount_sent} Liter</Text>
                                        </View>
                                    </View>
                                </View>
                            </View>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start', width: '100%' }}>
                        {errors.is_zakat_sent?.message ? <Caption style={{ color: 'red' }}> {errors.is_zakat_sent?.message}</Caption> : <View></View>}
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', width: '100%' }}>
                        <Controller
                            control={control}
