    ...
    if (state.loadingSelesaiReceived || !state.celenganSisaReceived) {
        ...
        return (
            <View style={styles.container}>
                ...
                <Subheading>Terkirim : {state.celenganSelesaiReceived.length}</Subheading>
                <FAB
                    style={styles.fab}
                    small
                    icon="refresh"
                    onPress={() => dispatch(GetStatusSelesaiMoneyBoxReceived(year_hijriah))}
                />
                <FlatList data={state.celenganSelesaiReceived} style={{ width: '90%', maxHeight: '80%' }}
