...
import { GetStatusSelesaiZakatFitrahReceived } from '../../../config/redux/services'
import { useDispatch, useSelector } from 'react-redux'
const SelesaiZakatFitrahReceived = ({ route, navigation, year_hijriah }) => {
    ...
    if (state.loadingSelesaiReceived || !state.zakatFitrahSisaReceived) {
        return (
    ...
    else if (state.zakatFitrahSelesaiReceived) {
        return (
            <View style={styles.container}>
                {/* <View style={styles.sub_container}> */}
                <Title style={{ marginVertical: 10 }}>Daftar Peserta Yang Telah Dikumpulkan</Title>
                <Subheading>Terkirim : {state.zakatFitrahSelesaiReceived.length}</Subheading>
                <FAB
                    style={styles.fab}
                    small
                    icon="refresh"
                    onPress={() => dispatch(GetStatusSelesaiZakatFitrahReceived(year_hijriah))}
                />
                <FlatList data={state.zakatFitrahSelesaiReceived} style={{ width: '90%', maxHeight: '80%' }}
        ...
        return (
            <View style={styles.container}>
                <View style={styles.sub_container}>
                    ...
                    <Title style={{ textAlign: 'center' }}>Semua Peserta Belum Diberikan zakatFitrahnya</Title>
...
export default SelesaiZakatFitrahReceived
