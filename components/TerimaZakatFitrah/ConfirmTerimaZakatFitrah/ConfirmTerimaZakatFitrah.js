...
const ConfirmTerimaZakatFitrah = ({ route, navigation }) => {
    ...
    const [dataPostZakatFitrah, setdataPostZakatFitrah] = useState(false)
    ...
    useEffect(() => {
        reset({
            ...
            id: state.zakatFitrahReceived ? state.zakatFitrahReceived[0].id : false,
            is_zakat_received: state.zakatFitrahReceived ? state.zakatFitrahReceived[0].is_zakat_received : false,
            amount_received: state.zakatFitrahReceived ? state.zakatFitrahReceived[0].amount_received.toString() : null,
            kulak: state.zakatFitrahReceived ? (state.zakatFitrahReceived[0].amount_received / 3.25).toString() : null,
            notes: state.zakatFitrahReceived ? state.zakatFitrahReceived[0].notes : null,
            year_hijriah: year_hijriah
        })
    }, [state.zakatFitrahReceived])

    const dispatchData = () => {
        dispatch(GetDetailZakatFitrahReceived({ id_peserta: id_peserta, year_hijriah: year_hijriah }))
    }
    const schema = yup.object().shape({
        amount_received: yup.number().typeError('Wajib angka').required('Wajib diisi'),
            ...
            < View style={styles.container} >
                <View style={styles.sub_container}>
                    ...
                    <Card style={styles.container_menu}>
                        <Card.Content>
                            <View style={{ flexDirection: 'column', justifyContent: 'space-around', width: '100%' }}>
                                <Title style={{ fontSize: 20, textAlign: 'center', color: 'white' }}> Identitas Peserta Pengumpulan Zakat Fitrah</Title>
                                ...
                                <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between' }}>
                                    <View style={{ flexDirection: 'column' }}>
                                        <View style={{ flexDirection: 'row' }}>
                                            ...
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.zakatFitrahReceived[0].id_peserta}</Text>
                                        </View>
                                        <View style={{ flexDirection: 'row' }}>
                                            <Text style={{ fontSize: 16, color: 'white' }}> Nama  </Text>
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.zakatFitrahReceived[0].id_peserta_peserta.name}</Text>
                                        </View>
                                    </View>
                                    <View style={{ flexDirection: 'column' }}>
                                        <View style={{ flexDirection: 'row' }}>
                                            <Text style={{ fontSize: 16, color: 'white' }}> Tahun  </Text>
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.zakatFitrahReceived[0].year_hijriah} H</Text>
                                        </View>
                                    </View>
                                </View>
                            </View>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start', width: '100%' }}>
                        {errors.is_zakat_received?.message ? <Caption style={{ color: 'red' }}> {errors.is_zakat_received?.message}</Caption> : <View></View>}
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', width: '100%' }}>
                        <Controller
                            control={control}
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start', width: '100%' }}>
                        {errors.amount_received?.message ? <Caption style={{ color: 'red', justifyContent: 'flex-start', alignItems: 'flex-start' }}> {errors.amount_received?.message}</Caption> : <View></View>}
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', width: '100%' }}>
                        <Controller
                            control={control}
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginVertical: '3%' }}>
                        <Button onPress={handleSubmit(onSubmit)} style={{ width: '100%' }} loading={state.loadingData} disabled={state.loadingData} mode="contained"><Text style={{ color: 'white' }}>Simpan</Text></Button>
                    </View>
                </View>
            </View >
        )
        ...
        return (
            <View style={styles.container}>
                <View style={styles.sub_container}>
                    ...
                    <Title style={{ textAlign: 'center' }}>Silakan menghubungi Admin E-DKM Al-Irfan</Title>
