...
import { Avatar, Button, Caption, Card, Divider, Snackbar, TextInput, Title } from 'react-native-paper'
    ...
    useEffect(() => {
        if (!user.user) {
            ...
            if (update && state.qurbanSent.length > 0) {
                ...
                dispatch(GetDetailQurbanSent({ id_peserta: id_peserta, year_hijriah: year_hijriah })).then((response) => {
                    setupdate(true)
                })
            }
            ...
            < View style={styles.container} >
                <View style={styles.sub_container}>
                    ...
                    <Card style={styles.container_menu}>
                        <Card.Content>
                            <View style={{ flexDirection: 'column', justifyContent: 'space-around', width: '100%' }}>
                                <Title style={{ fontSize: 20, textAlign: 'center', color: 'white' }}> Identitas Peserta Penerima Daging Qurban</Title>
                                ...
                                <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between' }}>
                                    <View style={{ flexDirection: 'column' }}>
                                        <View style={{ flexDirection: 'row' }}>
                                            ...
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.qurbanSent[0].id_peserta}</Text>
                                        </View>
                                        <View style={{ flexDirection: 'row' }}>
                                            <Text style={{ fontSize: 16, color: 'white' }}> Nama  </Text>
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.qurbanSent[0].id_peserta_peserta.name}</Text>
                                        </View>
                                    </View>
                                    <View style={{ flexDirection: 'column' }}>
                                        <View style={{ flexDirection: 'row' }}>
                                            <Text style={{ fontSize: 16, color: 'white' }}> Tahun  </Text>
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.qurbanSent[0].year_hijriah} H</Text>
                                        </View>
                                        <View style={{ flexDirection: 'row' }}>
                                            <Text style={{ fontSize: 16, color: 'white' }}> Jatah  </Text>
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.qurbanSent[0].amount_sent} {state.qurbanSent[0].amount_type}</Text>
                                        </View>
                                    </View>
                                </View>
                            </View>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', width: '100%' }}>
                        ...
                        <Controller
                            ...
                            render={({ field: { onChange, onBlur, value } }) => (
                                <RadioButton.Group onValueChange={value => handleChangeForm('is_qurban_sent', value)} value={dataPostQurban.is_qurban_sent.toString()} defaultValue={state.qurbanSent[0].is_qurban_sent.toString()}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start', width: '100%' }}>
                        {errors.is_qurban_sent?.message ? <Caption style={{ color: 'red' }}> {errors.is_qurban_sent?.message}</Caption> : <View></View>}
                    </View>
                    <Controller
                        control={control}
                        render={({ field: { onChange, onBlur, value } }) => (
                        ...
                        name="notes" />
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start', width: '100%' }}>
                        {errors.amount_sent?.message ? <Caption style={{ color: 'red' }}> {errors.amount_sent?.message}</Caption> : <View></View>}
                    </View>
