...
import { GetDetailCelenganSent, UpdateCelenganSent } from '../../config/redux/services'
...
const ConfirmBeriCelengan = ({ route, navigation }) => {
    ...
    useEffect(() => {
        if (!user.user) {
            ...
            dispatch(GetDetailCelenganSent({ id_peserta: id_peserta, year_hijriah: year_hijriah }))
        }
    }, [user])
    useEffect(() => {
        if (state.celenganSent) {
            reset({
                ...
                is_money_box_sent: state.celenganSent ? state.celenganSent[0].is_money_box_sent : false,
    ...
    if (state.loading || !state.celenganSent) {
        ...
        return (
            < View style={styles.container} >
                <View style={styles.sub_container}>
                    <Title style={{ marginVertical: 20 }}>Beri Celengan Dana Sosial</Title>
                                ...
                                <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between' }}>
                                    <View style={{ flexDirection: 'column' }}>
                                        <View style={{ flexDirection: 'row' }}>
                                            ...
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.celenganSent[0].id_peserta}</Text>
                                        </View>
                                        <View style={{ flexDirection: 'row' }}>
                                            <Text style={{ fontSize: 16, color: 'white' }}> Nama  </Text>
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.celenganSent[0].id_peserta_peserta.name}</Text>
                                        </View>
                                    </View>
                                    <View style={{ flexDirection: 'column' }}>
                                        <View style={{ flexDirection: 'row' }}>
                                            <Text style={{ fontSize: 16, color: 'white' }}> Tahun  </Text>
                                            <Text style={{ fontSize: 16, color: 'white' }}>: {state.celenganSent[0].year_hijriah} H</Text>
                                        </View>
                                    </View>
                                </View>
                            </View>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start', width: '100%' }}>
                        {errors.is_money_box_sent?.message ? <Caption style={{ color: 'red' }}> {errors.is_money_box_sent?.message}</Caption> : <View></View>}
                    </View>
                    <Controller
                        control={control}
                        render={({ field: { onChange, onBlur, value } }) => (
                        ...
                        name="notes" />
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start', width: '100%' }}>
                        {errors.notes?.message ? <Caption style={{ color: 'red' }}> {errors.notes?.message}</Caption> : <View></View>}
                    </View>
